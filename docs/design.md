...
- Logo 定稿确认：`playground-logo-final.html`
- Logo 字体探索过程：`playground-logo-v4.html`（24 种字体组合）
- Logo 可爱度调节：`playground-logo-v5.html`（12 档梯度）
...
- 视觉原型：`playground-visual-directions.html`（浏览器打开，选「锐 Sharp」查看效果）
- 六页面设计原型：`playground-pages.html`（总览 / Popup / 每日回味 / Content Script / 难句集 / 设置）
- Logo 定稿：`playground-logo-final.html`
- 灵感来源：`frontend-slides-main/` 项目的设计哲学

---

## 设计 Token

编码时直接用这些值。完整 CSS 参考 `playground-pages.html`。
...
完整 SVG data URI 见 `playground-pages.html` 中的 `.noise` class。
